...
/*
// function handler to handle the click and drag event (not functional)
const mouseClickAndDrag = (cursor, content, pstate, render) => {
	// track where the click happened
	content.addEventListener('mousedown', event => {
		// prepare the cursor for movement by making it absolute
		cursor.style.position = 'absolute';
		
		// centers the cursor at (pageX) coordinate
		function moveAt(pageX){
			ball.style.left = pageX;
		}
		
		// move cursor under the pointer
		moveAt(event.pageX);
	...
	// move the content as the cursor moves with respect to the original click
		content.addEventListener('mousemove', onMouseMove);
	
	
	// stop moving the contents when finished
...
module.exports = (content, desc) => {
  ...
  // modifying the view0 element
  els.view0.onmousedown = function(event) {

    // center the container under the pointer
    let shiftX = event.clientX - els.view0.getBoundingClientRect().left;
    
  	// (1) prepare for moving
  	els.view0.style.position = 'absolute';
  	els.view0.style.zIndex = 1000;
  	// els.view0.style.top = els.timetop.style.height + 'px';
  	
  	// move it in front of the body
  	document.body.append(els.view0);
  	
  	// centers container at (pageX);
  	function moveAt(pageX){
  		els.view0.style.left = pageX - shiftX + 'px';
  	}
  	
  	//absolutely position container under pointer
  	moveAt(event.pageX);
  	...
  	// (3) release on mouseup
  	els.view0.onmouseup = function() {
  		document.removeEventListener('mousemove', onMouseMove);
  		els.view0.onmouseup = null;
  	}
  }
  
  // modifying the timetop element
  els.timetop.onmousedown = function(event) {

    // center the container under the pointer
    let shiftX = event.clientX - els.timetop.getBoundingClientRect().left;
    
  	// (1) prepare for moving
  	els.timetop.style.position = 'absolute';
  	els.timetop.style.zIndex = 1000;
  	// els.view0.style.top = els.timetop.style.height + 'px';
  	
  	// move it in front of the body
  	document.body.append(els.timetop);
  	
  	// centers container at (pageX);
  	function moveAt(pageX){
  		els.timetop.style.left = pageX - shiftX + 'px';
  	}
  	...
  	// (3) release on mouseup
  	els.timetop.onmouseup = function() {
  		document.removeEventListener('mousemove', onMouseMove);
  		els.timetop.onmouseup = null;
  	}
  }
  
  // modifying the timebot element
  els.timebot.onmousedown = function(event) {

    // center the container under the pointer
    let shiftX = event.clientX - els.timebot.getBoundingClientRect().left;
    
  	// (1) prepare for moving
  	els.timebot.style.position = 'absolute';
  	els.timebot.style.zIndex = 1000;
  	// els.view0.style.top = els.timetop.style.height + 'px';
  	
  	// move it in front of the body
  	document.body.append(els.timebot);
  	
  	// centers container at (pageX);
  	function moveAt(pageX){
  		els.timebot.style.left = pageX - shiftX + 'px';
  	}
  	...
  	// (3) release on mouseup
  	els.timebot.onmouseup = function() {
  		document.removeEventListener('mousemove', onMouseMove);
  		els.timebot.onmouseup = null;
  	}
  }
  
  // disable the browser's click and drag functionality
  els.view0.ondragstart = function () {
    return false;
  }
  
  els.timetop.ondragstart = function () {
  	return false;
  }
  
  els.timebot.ondragstart = function () {
  	return false;
  }
  */

  // pinchAndZoom(els.container, content, pstate, render);
  //mouseClickAndDrag(els.cursor, els.container, pstate, render);
  //els.container.addEventListener('mousedown', mouseClickAndDrag(content, pstate, render));
